...
newy = np.array(newy).astype(np.float32)
...
def get_rating_loss(predicted, actual, users, sparse, makescale, sumall,
                    scalemin, scales, battrain, batval, validacao, idstrain, idsvalid):
    ...
    if not validacao:
        ...
        if len(idstrain) > init:
            ids_songs[:] = idstrain[init:end]
            battrain[0] += 1
    else:
        batchatual = batval[0]
        init = batchatual * 128
        end = init + 128
        if len(idsvalid) > init:
            ids_songs[:] = idsvalid[init:end]
            batval[0] += 1
    ...
    if len(ids_songs) > 0:
        ...
        songpandas = sparse[sparse['music'].isin(ids_songs)]
        ...
        if qtdusr > 0:
            ...
            for c in range(len(ids_songs)):
                if T.gt(tamanho_batch_predito, c):

                    pred_facts = predicted
                    if makescale:
                        pred_facts -= scalemin
                        pred_facts /= scales

                    all_rat_cnn_song = T.dot(newus_matrix, pred_facts.T)
                    all_rat_cnn_song = all_rat_cnn_song.T
                    all_rat_cnn_song = T.stack(all_rat_cnn_song)



                    all_rat_cnn_song = all_rat_cnn_song
                    if sumall:
                        all_rat_cnn_song = all_rat_cnn_song[(all_rat_cnn_song > 0).nonzero()]

                    predic_rat = T.sum(all_rat_cnn_song)

                    preds.append(predic_rat)
...
def get_rating_loss2(predicted, actual, users, sparse, makescale, sumall,
                    scalemin, scales, battrain, batval, validacao, idstrain, idsvalid):
...
def objective_ratings(layers, loss_function, target, users, sparse, makescale, sumall,
                      battrain, batval, idstrain, idsvalid, aggregate=aggregate, deterministic=False, get_output_kw=None):
    ...
    loss = loss_function(network_output, target, users, sparse, makescale,
                         sumall, minscale, scale, battrain, batval, validacao, idstrain, idsvalid)
...
def on_epoch_finish_callback_scale(net, hist):
    ...
    print "1-total de batchs treino: ", bat_train_atual[0]
    print "1-total de batchs valid: ", bat_eval_atual[0]

    print "2-total de batchs treino: ", net.objective_battrain[0]
    print "2-total de batchs valid: ", net.objective_batval[0]
...
net3 = NeuralNet(
    ...
    objective_idstrain=idcs_songs_train,
    objective_idsvalid=idcs_songs_valid,
